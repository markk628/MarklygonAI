...
class StockTradingEnv:
    ...
    def step(self, action):
        ...
        if invalid_action:
            reward = self.calculate_reward(action, False)
        else:
...
class DQNAgent:
    ...
    def __init__(
        ...
        # Memory setup
        if use_prioritized:
            self.memory = PrioritizedReplayBuffer(memory_size, alpha=per_alpha, beta=per_beta, beta_increment=per_beta_increment)
        else:
    ...
    def train(self):
        ...
        if self.epsilon > self.epsilon_min and self.total_steps * self.epsilon_decay_target_pct > 0:
             ...
             self.epsilon = self.epsilon_min + (self.epsilon - self.epsilon_min) * np.exp(-self.decay_rate_multiplier * self.current_step / decay_steps)
...
def main():
    ...
    agent = DQNAgent(
        ...
        epsilon_decay_target_pct=epsilon_decay_target_pct,
        batch_size=1024,      
        memory_size=20000,   
        update_frequency=4,  
    ...
    # evaluate on test setf
    print("\nEvaluating on test set...")
    evaluate_agent(test_env, agent, episodes=1)
