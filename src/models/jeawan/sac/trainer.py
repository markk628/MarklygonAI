...
def main():
    # Configuration
    ticker = 'AAPL'  # Change to your stock ticker
    ...
    training_results = train_sac(
        data_path=data_path,
        cutoff=cutoff,
        num_episodes=500,  # More episodes for SAC
        save_interval=50,
        validation_frequency=EVALUATE_INTERVAL,
        early_stopping_patience=15,  # Slightly more patience for SAC
    ...
    if 'avg_sharpe_ratio' in aggregate_stats:
        print(f"Average Sharpe Ratio: {aggregate_stats['avg_sharpe_ratio']:.2f}")
    if 'avg_max_drawdown' in aggregate_stats:
    ...
    if training_results.get('preprocessor') is not None:
        ...
        print(f"SAC Preprocessing Configuration: {training_results['preprocessor'].get_preprocessing_info()}")
