...
class TradingConfig:
    ...
    commission_rate: float = TRANSACTION_FEE_PERCENT
    max_position_size: float = 0.7 # 1.0  # Max fraction of balance to use per trade
    ...
    per_epsilon: float = 1e-3  # Increased from 1e-6 to prevent underflow
    ...
    # Features
    num_features: int = 48  # 40 market features + 8 portfolio features
    sequence_length: int = WINDOW_SIZE  # 60 minutes of historical data
...
class DuelingNetwork(nn.Module):
    ...
    Architecture:
    1. Market Branch (1D CNN):
       - Processes time series market data
       - Captures temporal patterns and trends
       - Uses convolutional layers for efficiency
       
    2. Portfolio Branch (Fully Connected):
       - Processes current portfolio state
       - Simple FC layers since portfolio state has no temporal dimension
       
    3. Combination:
       - Features from both branches are concatenated
       - Passed through shared layers
       - Split into value and advantage streams (Dueling architecture)
       
    This design is more efficient than repeating portfolio state across timesteps
    and allows the network to learn specialized representations for each data type.
    """
    ...
    def __init__(self, config: TradingConfig):
        ...
        # Market data branch - 1D CNN for time series processing
        # Input: (batch_size, sequence_length, num_market_features)
        # We'll permute to (batch_size, num_market_features, sequence_length) for Conv1d
        self.market_features = 40  # Number of market features (excluding portfolio features)
        
        self.market_branch = nn.Sequential(
            # First conv block
            nn.Conv1d(in_channels=self.market_features, out_channels=64, kernel_size=5, padding=2),
            nn.BatchNorm1d(64),
            nn.ReLU(),
        ...
        # Input: (batch_size, 8) - portfolio features
        self.portfolio_features = 8
        
        self.portfolio_branch = nn.Sequential(
            nn.Linear(self.portfolio_features, 64),
            nn.LayerNorm(64),
            nn.ReLU(),
            nn.Dropout(0.1),
        ...
        # Combined features: 256 (from market branch) + 64 (from portfolio branch) = 320
    ...
    def _initialize_weights(self):
        ...
        - Slow convergence
        """
        # Initialize market branch
        for layer in self.market_branch:
        ...
        for layer in self.value_stream:
            if isinstance(layer, nn.Linear):
                if layer == self.value_stream[-1]:  # Output layer
        ...
        for layer in self.advantage_stream:
            if isinstance(layer, nn.Linear):
                if layer == self.advantage_stream[-1]:  # Output layer
                    # Smaller initialization for output layer
                    nn.init.uniform_(layer.weight, -3e-4, 3e-4)
                    nn.init.constant_(layer.bias, 0)
                else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: State tensor of shape (batch_size, sequence_length, num_features)
               where num_features = market_features + portfolio_features
        """
        # Get dimensions
        # batch_size = x.size(0)
        # sequence_length = x.size(1)
        
        # Split market data and portfolio data
        # Market data: first 40 features of each timestep
        # Portfolio data: last 8 features (same for all timesteps, so we take from first timestep)
        market_data = x[:, :, :self.market_features]  # (batch_size, sequence_length, 40)
        portfolio_data = x[:, 0, self.market_features:]  # (batch_size, 8) - take from first timestep
        
        # Process market data through CNN
        # Conv1d expects (batch_size, channels, sequence_length)
        market_data = market_data.permute(0, 2, 1)  # (batch_size, 40, sequence_length)
        market_features = self.market_branch(market_data)  # (batch_size, 256, 1)
        market_features = market_features.squeeze(-1)  # (batch_size, 256)
        
        # Process portfolio data through FC layers
        portfolio_features = self.portfolio_branch(portfolio_data)  # (batch_size, 64)
        
        # Combine features
        combined_features = torch.cat([market_features, portfolio_features], dim=1)  # (batch_size, 320)
...
class PrioritizedReplayBufferGPU:
    """GPU-based Prioritized Experience Replay Buffer"""
    ...
    def __init__(self, capacity: int, config: TradingConfig, device: torch.device=DEVICE):
        ...
        self.states = torch.zeros((capacity, config.sequence_length, config.num_features), dtype=torch.float32, device=device)
        self.actions = torch.zeros(capacity, dtype=torch.long, device=device)
        self.rewards = torch.zeros(capacity, dtype=torch.float32, device=device)
        self.next_states = torch.zeros((capacity, config.sequence_length, config.num_features), dtype=torch.float32, device=device)
    ...
    def push(self, 
        ...
        # Store experience (convert types as needed)
    ...
    def sample(self, batch_size: int, beta: float) -> Tuple[torch.Tensor, ...]:
        ...
        # Sample indices with error handling
        try:
            indices = torch.multinomial(probs, batch_size, replacement=True)
        except RuntimeError as e:
            # Fallback to uniform sampling if multinomial fails
            print(f"Warning: Multinomial sampling failed, using uniform sampling. Error: {e}")
    ...
    def update_priorities(self, indices: torch.Tensor, td_errors: torch.Tensor):
        ...
        # Ensure priorities don't get too small (use 0.001 as minimum)
...
class TradingEnvironment:
    ...
    def __init__(self, data: pd.DataFrame, scaled_data: pd.DataFrame, config: TradingConfig, mode: str = 'train', device: torch.device=DEVICE):
        self.data = data
    ...
    def reset(self, start_idx: Optional[int] = None) -> torch.Tensor:
        ...
        self.invalid_actions = 0  # Track invalid action attempts
        
        # Set starting position
        if start_idx is not None:
            self.current_step = start_idx
        elif self.mode == 'train':
            # Random start point for training (ensure we have enough future data)
            max_start = max(self.config.sequence_length, 
                           len(self.data) - self.config.sequence_length - 100)
            if max_start > self.config.sequence_length:
                self.current_step = np.random.randint(self.config.sequence_length, max_start)
            else:
                self.current_step = self.config.sequence_length
        else:
            # For validation and testing, always start from the beginning
            self.current_step = self.config.sequence_length
    ...
    def _get_state(self) -> torch.Tensor:
        """Get current state as tensor including portfolio information"""
        # Get historical data
        start_idx = self.current_step - self.config.sequence_length + 1
        end_idx = self.current_step + 1
        
        # Extract market features
        market_data = self.scaled_data.iloc[start_idx:end_idx].values
        ...
        if not np.isfinite(current_price):
            print(f"Warning: Non-finite price in _get_state: {current_price}")
            current_price = 0.0
        ...
        market_state = torch.tensor(market_data, dtype=torch.float32, device=self.device)
        ...
        portfolio_state_repeated = portfolio_state.unsqueeze(0).repeat(self.config.sequence_length, 1)
        
        # Concatenate market and portfolio features
        combined_state = torch.cat([market_state, portfolio_state_repeated], dim=1)
    ...
    def step(self, action: int) -> Tuple[torch.Tensor, float, bool, Dict]:
        ...
        if not np.isfinite(self.balance):
            print(f"Warning: Non-finite balance detected: {self.balance}")
            self.balance = 0
        ...
        if not np.isfinite(current_price) or not np.isfinite(next_price):
            print(f"Warning: Non-finite price detected. Current: {current_price}, Next: {next_price}")
            # Skip this step
            self.current_step += 1
        ...
        if action == 1:  # Buy
            if self.position > 0:
                # Already holding position - invalid buy
                invalid_action = True
                self.invalid_actions += 1
                reward = -0.01  # Small penalty for invalid action
            elif self.balance <= 0:
                # No balance to buy - invalid buy
                invalid_action = True
                self.invalid_actions += 1
                reward = -0.01
            else:
                # Valid buy - check if we can afford it
            ...
            if self.position == 0:
                # No position to sell - invalid sell
                invalid_action = True
                self.invalid_actions += 1
                reward = -0.01
        ...
        if not invalid_action:
            if action == 1:  # Buy
                ...
                # Validate balance after trade
                if not np.isfinite(self.balance):
                    print(f"Warning: Balance became non-finite after buy: {self.balance}")
                    self.balance = 0
                    
            elif action == 2:  # Sell
                # Sell all shares
                revenue = self.position * current_price * (1 - self.config.commission_rate)
                cost_basis = self.position * self.entry_price * (1 + self.config.commission_rate)
                ...
                # Validate balance after trade
                if not np.isfinite(self.balance):
                    print(f"Warning: Balance became non-finite after sell: {self.balance}")
                    self.balance = 0
                ...
                if cost_basis > 0 and np.isfinite(profit) and np.isfinite(cost_basis):
                    ...
                    reward = percentage_return * 10  # Scale up for better learning
                else:
                    reward = 0
...
class DoubleDuelingDQN:
    ...
    def __init__(self, config: TradingConfig, device: torch.device=DEVICE):
        ...
        # Optimizer - AdamW is better for financial models (better regularization)
        # AdamW vs Adam comparison for financial trading:
        # 1. AdamW decouples weight decay from gradient-based updates
        # 2. Better generalization on financial time series (reduces overfitting)
        # 3. More stable training with noisy financial data
        # 4. Weight decay acts as L2 regularization, preventing large weights
        # 5. Particularly important for DQN where overfitting to recent experiences is common
        ...
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            ...
            patience=10,  # Wait 10 validations before reducing
    ...
    def train_episode(self, env: TradingEnvironment) -> Dict[str, float]:
        ...
        if not np.isfinite(final_value):
            print(f"Warning: Non-finite final value. Balance: {info['balance']}, Position: {info['position']}, Price: {info['current_price']}")
            final_value = 0.0
            
        # Calculate return safely
        if self.config.initial_balance > 0 and np.isfinite(final_value):
        ...
        if not np.isfinite(total_return):
            print(f"Warning: Non-finite return calculated. Final value: {final_value}, Initial: {self.config.initial_balance}")
            total_return = 0.0
...
def train_dqn(data_path: str,
    ...
    # Create environments
    train_env = TradingEnvironment(train_data, train_data_scaled, config, mode='train')
    val_env = TradingEnvironment(valid_data, valid_data_scaled, config, mode='val')
    test_env = TradingEnvironment(test_data, test_data_scaled, config, mode='test')
