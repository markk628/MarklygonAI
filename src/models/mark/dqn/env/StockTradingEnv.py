...
from src.models.mark.dqn.utils.RollingMinMaxScaler import RollingMinMaxScaler
...
class StockTradingEnv:
    ...
    def _is_out_of_game(self):
        return self.shares_held == 0 and self.balance * self.max_position_size < self.min_trade_amount
    ...
    def _get_state(self) -> NDArray:
        ...
        normalized_last_portfolio_value = self.last_portfolio_value / self.initial_balance if self.initial_balance > 0 else 0
        ...
        normalized_invalid_actions_count = self.invalid_actions / self.steps_per_episode
        # time-based states
        ...
        normalized_consecutive_holds = self.consecutive_holds / self.steps_per_episode
        normalized_consecutive_trades = self.consecutive_trades / (self.steps_per_episode / 2)
        normalized_total_trades = self.total_trades / (self.steps_per_episode / 2)
        encoded_last_action = float(self.last_action) / 2 if self.last_action is not None else 0.5
        
        is_buy_invalid_next_step = 1 if (self.shares_held > 0 and self.position_open) or self._is_out_of_game() else 0
        ...
        if self.use_hierarchical:
            ...
            position_management_metrics = np.array([
                ...
                normalized_invalid_actions_count,
                ...
                is_buy_invalid_next_step,
                is_sell_invalid_next_step
            ], dtype=np.float32)
        ...
        if done:
            ...
            if self._is_invalid_action(action):
                ...
                if trade_info.get('type') == 'BUY':
                    ...
                    buy_fee = self.entry_price * self.shares_held * self.transaction_fee
                    profit_pct = (trade_info['amount'] - trade_info['fee']) / (trade_info['entry_price'] * trade_info['shares']) - buy_fee # if trade_info['entry_price'] > 0 and trade_info['shares'] > 0 else 0
                    ...
                    if profit_pct > 0:
                        ...
                        if trade_info['current_trade_duration'] < self.steps_per_episode * 0.3:  # less than 30% of possible trade steps
    ...
    def create_rolling_min_max_scalers(self):
        states = ['portfolio_value', 'last_portfolio_value', 'balance', 'shares_value', 'shares_held', 
                  'avg_buy_price', 'position_pl', 'position_pl_ratio', 'profit_factor', 'consecutive_profits',
                  'consecutive_losses', 'sharpe_ratio', 'sortino', 'current_drawdown', 'proximity_to_critical_loss',
                  'profit_loss_opportunity_ratio', 'market_volatility', 'recent_price_change', 'price_acceleraiton',
                  'volatility_trend', 'highest_price', 'lowest_price', 'highest_price_and_lowest_price_ratio',
                  'highest_price_and_current_price_ratio', 'highest_price_and_entry_price_ratio',
                  'lowest_price_and_current_price_ratio', 'lowest_price_and_entry_price_ratio',
                  'position_utilization_kelly', 'invalid_actions', 'highest_price_since_buy', 'lowest_price_since_buy',
                  'highest_price_since_buy_and_lowest_price_since_buy_ratio', 'highest_price_since_buy_and_current_price_ratio'
                  'highest_price_since_buy_and_entry_price_ratio', 'lowest_price_since_buy_and_current_price_ratio',
                  'lowest_price_since_buy_and_entry_price_ratio', 'time_ratio_to_optimal', 'consecutive_holds',
                  'consecutive_trades', 'total_trades']
        
        return {state: RollingMinMaxScaler(self.window_size) for state in states}