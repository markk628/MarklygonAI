...
class PaperTradingBot:
    ...
    def __init__(self, model_id: int, initial_balance: float, max_position_size: float = 0.7):
        ...
        # Initialize portfolio history buffer
        self.portfolio_history = deque(maxlen=WINDOW_SIZE)
        ...
        # Initialize portfolio history with initial state (after everything else is loaded)
        # Note: This will use current_price = 0 initially, but will be updated with real data
                ...
                if self.last_data_timestamp and self.last_data_point:
                    ...
                    if time_since_last_data > 70:
                        ...
                        for i in range(1, min(minutes_to_fill + 1, self.MAX_GAP_FILL_MINUTES + 1)):
                            ...
                            # Update portfolio history for forward filled data
        ...
        for msg in msgs:
            ...
            if hasattr(msg, 'symbol'):
                ...
                if msg.symbol == self.ticker:
                    ...
                    # Update portfolio history with new price information (but no action taken)
                    if hasattr(self, 'portfolio_history'):
                        current_portfolio_state = self._calculate_current_portfolio_state()
                        self.portfolio_history.append(current_portfolio_state)
    ...
    def _prepare_state(self, df: pd.DataFrame) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            if self.model_type == "DQN":
                ...
                # Get portfolio history with proper padding if needed
                if len(self.portfolio_history) < WINDOW_SIZE:
                    # Pad with current portfolio state if history isn't full yet
                    padding_needed = WINDOW_SIZE - len(self.portfolio_history)
                    portfolio_features_history = np.array(
                        [portfolio_features] * padding_needed + list(self.portfolio_history)
                    )
                else:
                    portfolio_features_history = np.array(list(self.portfolio_history))
                
                # Ensure correct shape
                portfolio_features_history = portfolio_features_history.reshape(WINDOW_SIZE, 12)
                
                # Combine features
                combined_features = np.concatenate([stock_data, portfolio_features_history], axis=1)
