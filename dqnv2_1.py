...
class StockTradingEnv:
    ...
    def __init__(
        ...
        self.profit_threshold: float = 0.01      # profit-taking threshold 
        self.loss_threshold: float = -0.003      # loss-cutting threshold 
        self.trailing_stop: float = 0.008        # trailing stop value 
    ...
    def calculate_reward(self, action, next_state, done):
        ...
        portfolio_change = (current_portfolio_value - self.last_portfolio_value) / self.last_portfolio_value
        ...
        if self.shares_held > 0:
            ...
            if current_profit_pct > self.trailing_stop:
            ...
            if current_profit_pct < self.loss_threshold:
                reward += current_profit_pct * self.loss_penalty_weight
        ...
        if action == 1:  # hold
            ...
            hold_penalty = max(hold_penalty, self.hold_penalty_max)
            reward += hold_penalty
        else:
            self.consecutive_holds = 0
        ...
        if done:
            final_profit_pct = (current_portfolio_value - self.initial_balance) / self.initial_balance
            if final_profit_pct > 0:
                reward += final_profit_pct * self.profit_reward_weight * 2
            else:
                reward += final_profit_pct * self.loss_penalty_weight
    ...
    def step(self, action):
        ...
        trade_info = {}  # dictionary to store metrics about this step
        ...
        if action == 0:  # sell all
            if not can_trade:
                ...
                if sell_amount >= self.min_trade_amount:
            ...
            if self.position_open and self.shares_held > 0:
                ...
                if current_profit_pct < self.max_loss_threshold:
                    ...
                    self.trailing_stop_price = 0
                    self.position_open = False
                    reward -= self.stop_loss_penalty * abs(current_profit_pct)
        elif action == 2:  # buy as much as possible
            if not can_trade:
                reward = -0.1
                invalid_action = True
            elif self.balance > 0 and not self.position_open:
                max_shares = int(self.balance / (current_price * (1 + self.transaction_fee)))
                max_shares = min(max_shares, int(self.initial_balance * self.max_position_size / current_price))
        ...
        if self.balance + self.shares_held * current_price < self.initial_balance * 0.9:
            done = True
            reward -= 1
...
class DQNAgent:
    ...
    def train(self):
        ...
        #     next_q_values = torch.clamp(next_q_values, -10, 10)  # Q-value 클리핑
        
        # target_q_values = rewards + (self.discount_factor * next_q_values * (1 - dones))
        # target_q_values = torch.clamp(target_q_values, -10, 10)  # 타겟 Q-value 클리핑
...
def train_agent(env: StockTradingEnv, 
    ...
    for e in range(episodes):
        ...
        if validation_env is not None and (e + 1) % validation_frequency == 0:
            validation_score, _ = evaluate_agent(validation_env, agent, episodes=1, verbose=False)
...
def evaluate_agent(env: StockTradingEnv, agent: DQNAgent, episodes: int=10, verbose: bool=True):
    ...
    return (avg_return_rate if episodes > 1 else return_rate), all_metrics
...
def load_stock_data(ticker: str) -> pd.DataFrame:
    ...
    # return df[['close', 'volume', 'stochrsi_k_14_1min', 'stochrsi_d_14_1min', 'macd_12_26_9_1min', 'macd_signal_12_26_9_1min', 'macd_hist_12_26_9_1min',
    #            'roc_20_1min', 'mfi_21_1min', 'minute','minute_sin', 'minute_cos', 'hour', 'hour_sin', 'hour_cos', 'day', 'day_sin', 'day_cos',
    #            'month', 'month_sin', 'month_cos', 'quarter', 'quarter_sin', 'quarter_cos', 'close_rolling_std_15', 'volume_rolling_std_15', 'close_pct_change_1']]
    return df
